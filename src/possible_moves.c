...
char get_all_forward_moves_for_square(board *the_board, move *all_moves, char src_row, char src_col) {
    ...
    */
    ...
    for (enum directions d = 0; d < 6; d++) {
        char no_ally = 1;
        char no_enemy = 0;
        char temp = get_marb_dir(the_board, src_row, src_col, d, no_ally);
        while (no_ally < MAX_PUSHED_ALLY && temp == the_board->whose_turn + 1) {
            no_ally++;
            temp = get_marb_dir(the_board, src_row, src_col, d, no_ally);
        }
        no_ally--;
        if (temp == empty) {
            move m;
        ...
        while (no_enemy <= no_ally && temp == get_enemy_marble(the_board->whose_turn + 1)) {
            no_enemy++;
            temp = get_marb_dir(the_board, src_row, src_col, d, (no_ally + no_enemy));
        }
...
char get_all_lines_in_square(board *the_board, char src_row, char src_col, char (*all_lines)[4]) {
    ...
    for (enum directions d = 1; d < 4; d++) {
        ...
        char col = src_col;
        char no_ally = 1;
        char temp = get_marb_dir(the_board, row, col, d, no_ally);
        while (no_ally < MAX_LINE_LENGTH && temp == the_board->whose_turn + 1) {
            ...
            no_lines++;
            no_ally++;
            temp = get_marb_dir(the_board, row, col, d, no_ally);
        }
    }
    return no_lines;
}
...
char get_all_aside_moves_for_line(board *the_board, move *all_moves, const char line[4]) {
    ...
    for (enum directions move_d = 0; move_d < 6; move_d++) {
        ...
        char is_possible = 1;
        while (1) {
            char temp = get_marb_dir1(the_board, row, col, move_d);
            if (temp != empty) {
                is_possible = 0;
                break;
            }
